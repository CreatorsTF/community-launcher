...
module.exports = {
    ...
    //Sets up the module.
    Setup() {
        this.all_mods_data = JSON.parse(
            fs.readFileSync(
                path.resolve(__dirname, "..", "internal", "mods.json")
            )
        );
    ...
    ChangeCurrentMod(name) {
        return new Promise((resolve, reject) => {
            ...
            switch (this.currentModData.install.type) {
                case "jsonlist":
                    this.source_manager = new JsonListSource(
                        this.currentModData.install
                    );
                    break;
                case "github":
                    this.source_manager = new GithubSource(
                        this.currentModData.install
                    );
                    break;
                case "gamebanana":
                    this.source_manager = new GameBananaSource(
                        this.currentModData.install
                    );
                    break;
                default:
                    ...
                    reject(
                        "Mod install type was not recognised: " +
                            this.currentModData.install.type
                    );
                    return;
            }
            ...
            if (this.currentModVersion == null || this.currentModVersion == 0) {
                ...
                this.source_manager
                    ...
                    .then((version) => {
                        ...
                        else this.currentModState = State.INSTALLED;
    ...
    ModInstallPlayButtonClick() {
        global.log.log(
            "Install button was clicked! Reacting based on state: " +
                this.currentModState
        );
        if (this.currentModData != null) {
            switch (this.currentModState) {
                case State.NOT_INSTALLED:
                    ...
                    global.log.log(
                        "Will validate TF2 path before starting download..."
                    );
                    if (!ValidateTF2Dir()) {
                        this.FakeClickMod();
                        global.log.error(
                            "Ending Install attempt now as validation failed!"
                        );
                        return;
                    }
                    ...
                    this.source_manager
                        ...
                        .then((_url) => {
                            global.log.log(
                                "Successfuly got mod install file urls. Will proceed to try to download them."
                            );
                ...
                case State.UPDATE:
                    ...
                    global.log.log(
                        "Asking user if they want to update this mod."
                    );
                    this.source_manager
                        .GetLatestVersionNumber()
                        .then((version) => {
                            const update_msg = `Would you like to update this mod to version "${version}"`;
                ...
                default:
                    global.log.error(
                        "Somehow the install button was clicked when the mod is in the installed state."
                    );
                    break;
            }
        } else {
            this.FakeClickMod();
            ErrorDialog(
                "Mod data was not able to be read.\nPlease report this error.",
                "Mod Install Start Error"
            );
        }
    },
    ...
    UpdateCurrentMod() {
        ...
        this.source_manager
                ...
                if (version > this.currentModVersion) {
                    ...
                    if (this.currentModData.install.type == "jsonlist") {
                        ...
                        this.source_manager.GetJsonData().then((data) => {
                            const urls = [];
                            if (
                                data.hasOwnProperty("PatchUpdates") &&
                                data.PatchUpdates.length > 0
                            ) {
                            ...
                            if (urls.length > 0) {
                                global.log.log(
                                    "Incremental update will begin for current mod using the following archive urls: " +
                                        urls.toString()
                                );
                                this.ModInstall(urls).then(() => {
                                    ...
                                    SetNewModVersion(
                                        this.currentModVersionRemote,
                                        this.currentModData.name
                                    );
                                ...
                                global.log.warn(
                                    "Update source does not have patch data! Will have to download again fully."
                                );
                                this.source_manager
                                    .GetFileURL()
                                    .then((_url) => {
                        ...
                        this.source_manager.GetFileURL().then((_url) => {
                            global.log.log(
                                "Mod is type GitHub, will update using the most recent release url: " +
                                    _url
                            );
                            this.ModInstall(_url).then(() => {
                                SetNewModVersion(
                        ...
                        global.log.error(
                            "Unknown mod type found during update attempt."
                        );
                        ErrorDialog(
                            "Unknown mod type found during update attempt.",
                            "Error"
            ...
            .catch((e) => {
                ErrorDialog(e, "Mod Update Error");
            });
    },
            ...
            DownloadFiles_UI(urlArray)
                .then((files) => {
                    this.InstallFiles(files)
                        ...
                        .catch((e) => {
                            ErrorDialog(e, "Mod Install Error");
                            this.FakeClickMod();
                        });
                })
                .catch((e) => {
                    ErrorDialog(e, "Mod Files Download Error");
                    this.FakeClickMod();
                });
        });
    },
    ...
    SetupNewModAsInstalled() {
        ...
        const versionUpdated = SetNewModVersion(
            this.currentModVersionRemote,
            this.currentModData.name
        );
        ...
        if (!versionUpdated)
            global.config.current_mod_versions.push({
    ...
    RemoveCurrentMod() {
        ...
        const files_object = filemanager.GetFileListSync(
            this.currentModData.name
        );
        ...
        if (files_object.files != null && files_object.files.length > 0) {
            ...
            for (let i = 0; i < files_object.files.length; i++) {
                ...
                if (fs.existsSync(files_object.files[i]))
                    fs.unlinkSync(files_object.files[i]);
            ...
            setTimeout(() => {
                ...
                for (
                    let i = 0;
                    i < global.config.current_mod_versions.length;
                    i++
                ) {
                    const element = global.config.current_mod_versions[i];
                    if (
                        element.name &&
                        element.name == this.currentModData.name
                    ) {
            ...
            dialog.showMessageBox(global.mainWindow, {
                ...
                message:
                    "Mod cannot be removed. Please try to remove them manually.",
                buttons: ["OK"],
            });
        }
    },
    ...
    GetRealTF2Path() {
        ...
        return path.normalize(
            realPath.replace("{tf2_dir}", global.config.tf2_directory)
        );
    },
    ...
    InstallFiles(files) {
        return new Promise((resolve, reject) => {
            ...
            const entryProcess = () => {
                ...
                func(this.GetRealTF2Path(), entry.value[1], this.currentModData)
                    .then(() => {
                        entryIndex++;
    ...
    return new Promise((resolve, reject) => {
        ...
        const headersFunction = (headers) => {
            ...
            progressBar
                ...
                .on("progress", (value) => {
                    try {
                        progressBar.detail = `[File ${currentIndex + 1} of ${
                            urls.length
                        }] Downloaded ${(
                            Math.round((value / 1000000) * 100) / 100
                        ).toFixed(2)} MB out of ${maxProgressVal} MB.`;
                    } catch (e) {
                        reject(e.toString());
                    }
                });
            ...
            maxProgressVal =
                Math.round((parseInt(contentLength) / 1000000) * 100) / 100;
        };

        //Setup download sequence for all files
        const downloadFunc = () => {
            global.log.log(
                "Starting Download for file at: " + urls[currentIndex]
            );
            DownloadFile(urls[currentIndex], progressFunction, headersFunction)
                .then((file) => {
                    files.push(file);
                    currentIndex++;
...
function WriteZIPsToDirectory(targetPath, zips, currentModData) {
    ...
    //Load file list object
    const files_object = filemanager.GetFileListSync(currentModData.name);
    ...
    const Write = (name, d) => {
        const fullFilePath = path.join(targetPath, name);
    ...
    const HandleFile = (relativePath, file) => {
        inProgress++;
        if (file.dir) {
            const directory = path.join(targetPath, file.name);
        ...
        let headersFunction = (headers) => {
            ...
            progressBar
                ...
                .on("progress", function (value) {
                    progressBar.detail = `Downloaded ${(
                        Math.round((value / 1000000) * 100) / 100
                    ).toFixed(2)} MB out of ${maxProgressVal} MB.`;
                });
        };

        DownloadZIP(_url, progressFunction, headersFunction)
            .then((zip) => {
                progressBar.setCompleted();
                progressBar.close();
                resolve(zip);
...
function ValidateTF2Dir() {
    ...
    if (!global.config) {
        ErrorDialog(
            "The application could not load the config. It may have failed to write it to disk.\nPlease report this issue!",
            "Internal Error"
        );
        return false;
    }
    ...
    if (global.config.tf2_directory == "") {
        ErrorDialog(
            "No TF2 path has been specified. Please manually enter this in the Settings.\nE.g. 'C:\\Program Files (x86)\\steam\\steamapps\\common\\Team Fortress 2\\'",
            "TF2 Path Error"
        );
        return false;
    }
    ...
    if (!fs.existsSync(global.config.tf2_directory)) {
        ErrorDialog(
            "The current TF2 directory specified does not exist. Please check your settings.",
            "TF2 Path Error"
        );
        return false;
    }
    ...
    if (plat == "win32") {
        ...
        if (
            fs.existsSync(path.join(global.config.tf2_directory, "hl2_linux"))
        ) {
            return true;
        }
    }
    ...
    const appid_path = path.join(
        global.config.tf2_directory,
        "steam_appid.txt"
    );
    if (fs.existsSync(appid_path)) {
        const content = fs.readFileSync(appid_path, "utf-8");
    ...
    ErrorDialog(
        "The current TF2 directory specified does exist, but it did not pass validation.\nCheck it links only to the 'Team Fortress 2' folder and not to the sub 'tf' folder.\nPlease check your settings.",
        "TF2 Validation Error"
    );
    return false;
}
...
function DownloadFile(_url, progressFunc, responseHeadersFunc) {
    return new Promise((resolve, reject) => {
        ...
        const DoRequest = (__url, retries) => {
            ...
            const req = https.get(__url, (res) => {
                if (res.statusCode == 302) {
                    ...
                    res.on("end", () => {
                        ...
                        global.log.log(
                            "File download finished. Returning raw data."
                        );
            ...
            req.on("error", (err) => {
                global.log.error(
                    `File download request for ${_url} errored out: ` + err
                );
                reject(err);
            });
        };
...
function SetNewModVersion(version, currentModName) {
    ...
    for (let i = 0; i < global.config.current_mod_versions.length; i++) {
        const modVersionObject = global.config.current_mod_versions[i];
        if (modVersionObject.name == currentModName) {
            global.log.log(
                `Mod ${currentModName} version was updated from ${modVersionObject.version} to ${version}`
            );
...
function FatalError(errorMessage) {
    dialog
        ...
        .then(() => {
            global.log.error(
                "A fatal error was encountered! Program quit. Reason: " +
                    errorMessage
            );
            global.app.quit();
        });
...
functionMap.set("rar", () => {
    FatalError(
        "Cannot handle .rar files currently. This should not happen. Exiting..."
    );
});
functionMap.set("7z", () => {
    FatalError(
        "Cannot handle .7z files currently. This should not happen. Exiting..."
    );
});
functionMap.set("exe", () => {
    FatalError(
        "Downloaded file was windows executable. This should not happen, exiting. File was not written."
    );
});
