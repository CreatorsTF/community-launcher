...
module.exports.GameBananaSource = class GameBananaSource extends ModInstallSource {
    ...
    GetModPageData(){
        return new Promise((resolve, reject) => {
            GetWebPage(this.mod_page_url).then((pageString) => {
            let downloadPageUrlResult = pageString.match(downloadPageURLRegex);
                if(downloadPageUrlResult != null & downloadPageUrlResult.length > 0){
                    //Get capture group 1 to already remove the surrounding "s.
                    let capture1 = downloadPageUrlResult[2];
                    if(capture1 != null && capture1 != ""){
                        this.mod_download_page_url = capture1;
                        console.log("Game Banana: Found mod download page url: " + capture1);

                        GetWebPage(this.mod_download_page_url).then((dlPageString) => {

                            //Use regex to get the download url.
                            let downloadURL = dlPageString.match(downloadLinkRegex);

                            //Use another reg ex to locate the files release date to use as a version.
                            let fileDate = dlPageString.match(fileDateRegex);
                            this.latest_version = Date.parse(fileDate[1]);
                            ...
                            if(downloadURL != null && downloadURL != ""){
                                ...
                                reject("Failed to find mod file url on download page.");
                            }
                        });
                    }
                    else{
...
function GetWebPage(_url){
    return new Promise((resolve, reject) => {
        ...
        var data = [];
        let req = https.get(_url, options, res => {
        console.log(`statusCode: ${res.statusCode}`);
            ...
            res.on("end", function () {
                ...
                var pageContentStr = buf.toString("utf-8");
