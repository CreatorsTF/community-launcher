...
module.exports.GithubSource = class GithubSource extends ModInstallSource {
            ...
            this._GetGithubData()
                .then((data) => {
                    ...
                    if (releaseAssets != null && releaseAssets != []) {
                        ...
                        if (this.data.hasOwnProperty("asset_index"))
                            asset = releaseAssets[this.data.asset_index];
                        ...
                        reject(
                            "This Github repositorys latest release was missing a usable asset."
                        );
                    } else
                        reject(
                            "This Github repository has no releases avaliable."
                        );
                })
                .catch(reject);
        });
    ...
    _GetGitHubReleaseData() {
        return new Promise((resolve, reject) => {
            ...
            const url =
                github_api_url +
                `repos/${this.data.owner}/${this.data.name}/releases`;
            const options = {
                headers: {
