...
const moddata = JSON.parse(
    fs.readFileSync(path.resolve(__dirname, "internal", "mods.json"))
);
...
window.log.transports.console.format =
    "[{d}-{m}-{y}] [{h}:{i}:{s}T{z}] -- [{processType}] -- [{level}] -- {text}";
window.log.transports.file.format =
    "[{d}-{m}-{y}] [{h}:{i}:{s}T{z}] -- [{processType}] -- [{level}] -- {text}";
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    request.open(
        "GET",
        "https://api.github.com/repos/ampersoftware/Creators.TF-Community-Launcher/releases/latest"
    );
    request.send();
    request.onload = () => {
